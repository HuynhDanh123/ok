...
public class ClientThread implements Runnable {
	...
	public ClientThread(Socket socket, ChatBox main) {
		this.main = main;
		this.socket = socket;
		try {
	...
	public void run() {
		try {
			while (!Thread.currentThread().isInterrupted()) {
				...
				// hàm này sẽ thông báo đến client rằng có một file nhận, Chấp nhận hoặc từ chối
				// file
				case "CMD_FILE_XD": // Format: CMD_FILE_XD [sender] [receiver] [filename]
					...
					if (confirm == 0) { // client chấp nhận yêu cầu, sau đó thông báo đến sender để gửi file
						...
						try {
							...
							/*
							 * hàm này sẽ tạo một socket filesharing để tạo một luồng xử lý file đi vào và
							 * socket này sẽ tự động đóng khi hoàn thành.
							 */
							...
							/* Run Thread for this */
							new Thread(new nhanFileThread(fSoc, main)).start();
						} catch (IOException e) {
						...
						try {
							...
							String format = "CMD_SEND_FILE_ERROR " + sender
									+ " Người dùng từ chối yêu cầu của bạn hoặc bị mất kết nối.!";
							dos.writeUTF(format);
						} catch (IOException e) {
							System.out.println("[CMD_FILE_XD]: " + e.getMessage());
						}
