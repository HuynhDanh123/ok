...
public class nhanFileThread implements Runnable {

	protected Socket socket;
	protected DataInputStream dis;
	protected DataOutputStream dos;
	protected ChatBox main;
	protected StringTokenizer st;
	protected DecimalFormat df = new DecimalFormat("##,#00");
	private final int BUFFER_SIZE = 100;

	public nhanFileThread(Socket soc, ChatBox m) {
		this.socket = soc;
		this.main = m;
		try {
	...
	public void run() {
		try {
			while (!Thread.currentThread().isInterrupted()) {
				...
				// hàm này sẽ xử lý việc nhận một file trong một tiến trình nền xử lý từ một
				// user khác
				case "CMD_SENDFILE":
					...
					try {
						...
						/*
						 * Gửi lại thông báo lỗi đến sender Định dạng: CMD_SENDFILERESPONSE [username]
						 * [Message]
						 */
						DataOutputStream eDos = new DataOutputStream(socket.getOutputStream());
						eDos.writeUTF(
								"CMD_SENDFILERESPONSE " + consignee + " Kết nối bị mất, vui lòng thử lại lần nữa.!");
